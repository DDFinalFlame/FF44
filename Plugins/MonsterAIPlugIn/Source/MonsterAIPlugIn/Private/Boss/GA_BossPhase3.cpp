...
void UGA_BossPhase3::Tick_Rock()
{
    ...
    // ── 사운드 위치 추정: 아래로 라인트레이스해서 지면 ImpactLoc 구함 ──
    FVector ImpactLoc = XY; // 폴백
    {
        ...
        else      ImpactLoc = FVector(XY.X, XY.Y, Boss->GetActorLocation().Z); // 폴백
    }

    // ── 소환 시점 기준 고정 딜레이로 사운드 재생 예약(람다 X) ──
    if (RockImpactSound && RockImpactSfxDelay > 0.f)
    {
        FTimerHandle Th;
        FTimerDelegate Dlg;
        // UFUNCTION 바인딩 – 위치만 넘겨줍니다.
...
void UGA_BossPhase3::PlayImpactSfxAt(FVector ImpactLoc)
{
    ...
    // 여기서 굳이 Boss 유효성 확인 없이 this 컨텍스트로 재생해도 OK
