...
protected:
	...
	float CastInterval = 0.5f;                                 // 캐스팅 주기
...
protected:
	...
	UPROPERTY(EditDefaultsOnly, Category = "Phase|FallingRock")
	int32 RocksPerCastMin = 1;

	UPROPERTY(EditDefaultsOnly, Category = "Phase|FallingRock")
	int32 RocksPerCastMax = 2;

	UPROPERTY(EditDefaultsOnly, Category = "Phase|FallingRock")
	float SpawnHeight = 1200.f;                      // 지면 위 스폰 높이

	UPROPERTY(EditDefaultsOnly, Category = "Phase|FallingRock")
	float PlayerAreaRadiusMin = 100.f;

	UPROPERTY(EditDefaultsOnly, Category = "Phase|FallingRock")
	float PlayerAreaRadiusMax = 300.f;

	UPROPERTY(EditAnywhere, Category = "Phase1|SFX")
	USoundBase* RockImpactSound = nullptr;

	UPROPERTY(EditAnywhere, Category = "Phase1|SFX", meta = (ClampMin = "0.0"))
	float RockImpactSfxDelay = 1.8f; // n초 지연 (원하는 값)
